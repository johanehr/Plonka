<?php
  ...
  if (isset($_POST['name']) && isset($_POST['email']) && isset($_POST['phone']) && isset($_POST['psw1']) && isset($_POST['psw2'])  && isset($_POST['personal_number'])  && isset($_POST['conditions'])){
    ...
    $birthday = substr($personal_number,0,-4); // YYYYMMDDNNNN -> YYYYMMDD
    $birthday_DateTime=DateTime::createFromFormat('Ymd', $birthday);
    ...
    switch ($registration_status) {
      case 0:
        ...
        if (mysqli_connect_errno($con)) {
          ...
          if($stmt->execute()){
            echo 'Registration successful! You can now log in.'.PHP_EOL;
          }
          else{
            echo 'Error! Registration failed - do you already have an account?'.PHP_EOL;
          }
          $stmt->close();
          mysqli_close($con);
        }
    ...
    echo 'ERROR! POST-error. Please contact us via email.'.PHP_EOL;
  }
